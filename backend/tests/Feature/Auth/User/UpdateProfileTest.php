...
class UpdateProfileTest extends TestCase
{
    public function test_update_profile_changes_name_and_email(): void
    {
        ...
        $res->assertJsonStructure(['user' => ['id', 'name', 'nickname', 'email', 'created_at']]);
        $this->assertArrayNotHasKey('password', $res->json('user'));
    }

    public function test_update_profile_sets_and_returns_nickname(): void
    {
        $user = User::query()->create([
            'name' => 'Test User',
        ...
        $res->assertOk();
        $res->assertJsonPath('user.nickname', 'MyNick');
        $this->assertSame('MyNick', $user->fresh()->nickname);
    }
    ...
    public function test_update_profile_returns_422_when_nickname_too_long(): void
    {
        $user = User::query()->create([
            'name' => 'Test User',
            'email' => 'test@example.com',
